...
void init_gdtidt(void);//初始化全局段号记录表&中断记录表
void set_segmdesc(struct SEGMENT_DESCRIPTOR *sd, unsigned int limit, int base, int ar);//设置段号记录表
void set_gatedesc(struct GATE_DESCRIPTOR *gd, int offset, int selector, int ar);//设置中断记录表
void load_gdtr(int limit, int addr);//因为c语言不能直接给gdtr赋值，所以需要借助汇编
void load_idtr(int limit, int addr);//因为c语言不能直接给idtr赋值，所以需要借助汇编
